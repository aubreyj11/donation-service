...
const resolvers = {
  ...
  Mutation: {
    // Resolver for the createPaymentIntent mutation
    createPaymentIntent: async (parent, { amount, userId }, context) => {
      try {
        ...
        // Return the saved donation details and the client secret from the payment intent
    ...
    updateUser: async (parent, { avatar }, context) => {
      ...
      try {
        // Check if the user is authenticated
        if (context.user) {
          // Find the user in the database and update their avatar
